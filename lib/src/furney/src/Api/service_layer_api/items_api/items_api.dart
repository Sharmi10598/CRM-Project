...
class ItemsAPi{
  ...
  static Future<ItemModal> getGlobalData(String pack) async {
    log(URL.url+"Items?\$select=ItemCode,ItemName,SalesUnit,ItemPrices,U_Pack_Size,U_Tins_Per_Box&\$filter=((contains(ItemCode,'$searchData') or contains(ItemName,'$searchData')) and contains(U_Pack_Size,'$pack.') And contains(U_Prd_MainGrp,'$mainGroup') and contains(U_Prd_SubGrp,'$subGroup')and Valid eq 'tYES' )");
    try {
    final response = await http.get(
        Uri.parse(
      URL.url+"Items?\$select=ItemCode,ItemName,SalesUnit,ItemPrices,U_Pack_Size,U_Tins_Per_Box&\$filter=((contains(ItemCode,'$searchData') or contains(ItemName,'$searchData')) and contains(U_Pack_Size,'$pack.') And contains(U_Prd_MainGrp,'$mainGroup') and contains(U_Prd_SubGrp,'$subGroup')and Valid eq 'tYES' )" 
      ...
      if (response.statusCode == 200) {
        ...
        print(response.body);
        return ItemModal.fromJson(json.decode(response.body)as Map<String,dynamic>);
      } else {
