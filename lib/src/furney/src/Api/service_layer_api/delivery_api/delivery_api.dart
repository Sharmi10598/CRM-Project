...
class DeliveryAPi{
  ...
  static Future<DeliveryModal> getGlobalData() async {
    ...
    final response = await http.get(
        Uri.parse(
      URL.url+"DeliveryNotes?\$select=DocEntry,DocNum,DocDate,CardCode,CardName&\$filter= DocumentStatus eq 'bost_Open'",
      ),
        headers: {
      ...
      //  print(json.decode(response.body));
        return DeliveryModal.fromJson(json.decode(response.body)as Map<String,dynamic>);
      } else {
         print(json.decode(response.body));
  ...
  static Future<DeliveryModal> searchData() async {
    ...
    final response = await http.get(
        Uri.parse(//http://102.69.164.12:50001/b1s/v1/DeliveryNotes(10)
      URL.url+"DeliveryNotes?\$select=DocEntry,DocNum,DocDate,CardCode,CardName&\$filter=(contains(CardCode,'$searchValue') or contains(CardName,'$searchValue')) and DocumentStatus eq 'bost_Open'",
