  ...
  static Future<SoLogindata> getGlobalData() async {
    try {
      ...
      if (response.statusCode <= 210) {// || response.statusCode <= 210
        print("relogin: "+json.decode(response.body).toString());
        print(response.statusCode);
        return SoLogindata.fromJson(json.decode(response.body)as Map<String,dynamic>);
      } else {
          print("relogin: "+json.decode(response.body).toString());
        print(response.statusCode);
       // throw Exception('Err');
       return SoLogindata.error(json.decode(response.body)as Map<String,dynamic>);
      }
    } catch (e) {
