  ...
  static Future<int> getGlobalData() async {
    try {
      log(
        "CustoemrSales::" +
            URL.reportUrl +
            '$methodName/$slpCode,$fromDate,$toDate',
      );
      final response = await http.get(
        Uri.parse(
          URL.reportUrl +
              '$methodName/$slpCode,$fromDate,$toDate', //866   $docEntry
        ),
        headers: {
          'content-type': 'application/octet-stream',
        },
      );
      log('CustoemrSales.statusCode::${response.statusCode}');
      if (response.statusCode == 200) {
        ...
        final file =
            await File('${tempDir.path}/SalesInDay-$slpCode.pdf').create();
