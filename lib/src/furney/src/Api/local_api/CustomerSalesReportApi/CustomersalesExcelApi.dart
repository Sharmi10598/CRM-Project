...
class CustomerSaleExcelReportAPi {
  ...
  static Future<int> getGlobalData() async {
    try {
      log(
        "reportName::" +
            URL.reportUrl +
            '$reportName/$slpCode,$fromDate,$toDate',
      );
      final response = await http.get(
        Uri.parse(
          URL.reportUrl +
              '$reportName/$slpCode,$fromDate,$toDate', //866   $docEntry
        ),
        headers: {
          'content-type': 'application/octet-stream',
        },
      );
      log("reportNameSts: " + response.statusCode.toString());
