...
class ReportExcelApi{
  ...
  static Future<int> getGlobalData(String content, String methodname) async {
    try {
      log("Adherence excel Api"+ URL.reportUrl+ content);
    final response = await http.get(
     ...
     URL.reportUrl+ content,
      ...
      if (response.statusCode == 200) {
         ...
         log("Uint8List bytes: "+response.bodyBytes.toString());
           final bytes =  response.bodyBytes;
       final tempDir = await getTemporaryDirectory();
     final file = await File('${tempDir.path}/$methodname-${GetValues.slpCode}.xlsx').create();
