...
class CustomerStatementApi{
  ...
  static Future<int> getGlobalData(String? custCode,String? fromDate,String? toDate) async {
    try {
      log( URL.reportUrl+ 'NetPOrder/$custCode,$fromDate,$toDate');
    final response = await http.get(
        Uri.parse(
      URL.reportUrl+ 'NetPOrder/$custCode,$fromDate,$toDate', //866   $docEntry
      ),
        headers: {
      ...
      if (response.statusCode == 200) {
       ...
       final file = await File('${tempDir.path}/Customer Statement-$custCode.pdf').create();
