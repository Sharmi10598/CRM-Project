...
class ContentCreationState extends State<ContentCreation> {
  ...
  static List<AddQuotItem> itemsDetails = [];
                                    ...
                                    SpecialDiscountAPi.getGlobalData()
                                      ...
                                      if (value.specialValue!.isNotEmpty ||
                                        ...
                                        CreateDetailsState.contentAddItems =
                                            false;
                                      } else if (value.specialValue!.isEmpty ||
                                          value.specialValue!.isEmpty) {
                                        setState(() {
                                          loadingscrn = false;
                                        });
                                        mycontroller[1].text =
  ...
  Future validation4Alert(BuildContext context, int i) async {
    if (formkey[0].currentState!.validate()) {
      ...
      final price = double.parse(mycontroller[1].text);
      final qty = int.parse(mycontroller[2].text);
      final discountper =
          double.parse(mycontroller[3].text == '' ? '0' : mycontroller[3].text);
      final discount = (price * qty) * discountper / 100;
      final taxper = taxSelected;
      final tax = ((qty * price) - discount) * taxper / 100;
      ...
      itemsDetails.add(
        AddQuotItem(
          ...
          uPackSize: filteritemValue[i].U_Pack_Size,
          uTinsPerBox: filteritemValue[i].U_Tins_Per_Box,
          taxPer: taxper,
      ...
      if (itemsDetails.isNotEmpty) {
        for (var i = 0; i < itemsDetails.length; i++) {
          ...
          basictotalx = basictotalx +
              (itemsDetails[i].price! * itemsDetails[i].qty!.toDouble());
          discount = discount +
              ((itemsDetails[i].price! * itemsDetails[i].qty!.toDouble()) *
                  (itemsDetails[i].discounpercent! / 100));
          tax = tax + itemsDetails[i].tax!;
        }
  ...
  Future<dynamic> validation4AlertUpdate(BuildContext context, int i) async {
    if (formkey[1].currentState!.validate()) {
      ...
      final price = double.parse(mycontroller[4].text);
      final qty = int.parse(mycontroller[5].text);
      final discountper = double.parse(mycontroller[6].text);
      final discount = (price * qty) * discountper / 100;

      final taxper = taxSelected;
      log('taxSelectedtaxSelected:::$taxSelected');
      final tax = ((qty * price) - discount) * taxper / 100;
      final total = (price * qty) - discount + tax;
      log('totaltotaltotal::$total');
      final valueAfdisc = (qty * price) - discount;
      var carton2 = 0;
      ...
      if (itemsDetails[i].uPackSize! < 10 && itemsDetails[i].uTinsPerBox! > 0) {
        carton2 = qty ~/ itemsDetails[i].uTinsPerBox!;
      }
