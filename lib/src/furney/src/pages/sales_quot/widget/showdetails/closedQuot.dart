...
class ClosedQuot extends StatefulWidget {// required this.title
  const ClosedQuot({ Key? key, }) : super(key: key);
  // final String title;
  @override
 ...
 void getmoredata(){
      SalesQuotCancelAPi.callNextLink().then((val) {
        ...
        for (var i =0; i<lenthofList; i++ ){
            filterSalesValue.add(SalesQuotCancelValue(DocDate: 
            val.salesQuotcancelValue![i].DocDate,
             cardCode: 
             val.salesQuotcancelValue![i].cardCode,
         cardName:
         val.salesQuotcancelValue![i].cardName,
          docEntry:
          val.salesQuotcancelValue![i].docEntry,
          DocNum: 
          val.salesQuotcancelValue![i].DocNum, cancelStatus: val.salesQuotcancelValue![i].cancelStatus, 
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      //       ) :
            SafeArea(
        child: Padding(
          padding: EdgeInsets.symmetric(vertical: Screens.heigth(context)*0.005,horizontal: Screens.width(context)*0.02),
            child: Container(
              child: Column(
                children: [
                  Stack(
              ...
              Expanded(
                child: ListView.builder(
                  controller: scrollController,
                    itemCount: filterSalesValue.length,//dataSocialMedia.length,
                    itemBuilder: (context, i) {
                      ...
                      return Card(
                          ...
                          child: InkWell(
                            onTap: () {
                           SalesOuotDetailsAPi.docEntry = filterSalesValue[i].docEntry.toString();
                           print( SalesOuotDetailsAPi.docEntry);
                        SalesQtDetailsState.status = filterSalesValue[i].cancelStatus=='csYes'&&filterSalesValue[i].documentStatus=='bost_Close' ?'Cancelled'
                                    :filterSalesValue[i].cancelStatus=='csNo'&&filterSalesValue[i].documentStatus=='bost_Close' ?'Closed':'';
                         ...
                         mainAxisAlignment: MainAxisAlignment.spaceBetween,
                          children: [
                           ...
                           //   color: Colors.greenAccent,
                              ...
                              child: Text('${filterSalesValue[i].DocNum}',
                            ...
                            //   color: Colors.blue,
                              ...
                              child: Text('${filterSalesValue[i].cardName}',
                              style: TextStyles.bodytextBlack1(context),),
                            )
                          ],
                        ),
                             ...
                             //    color: Colors.red,
                                ...
                                child: Column(
                                  children: [
                                    ...
                                    :filterSalesValue[i].cancelStatus=='csNo'&&filterSalesValue[i].documentStatus=='bost_Close' ?'Closed':'', style: TextStyles.bodytextBlack1(context),),
                                    Text('${filterSalesValue[i].DocDate}' ,style:TextStyles.bodytextBlack1(context),),
                                  ],
                                ),
                              ),
