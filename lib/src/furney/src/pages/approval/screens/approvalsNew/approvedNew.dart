...
import 'package:ultimate_bundle/src/furney/src/Api/service_layer_api/approvals_api/approvedApi.dart';
...
class ApprovedNew extends StatefulWidget {
  const ApprovedNew({Key? key  }) : super(key: key);
  // final String title;  this.title
  @override
  ApprovedNewState createState() => ApprovedNewState();
}

class ApprovedNewState extends State<ApprovedNew> {
  ...
  bool isapprovalslengthNull=true;
   void getmoredata(){
      ...
      ApprovalsAPi.callNextLink().then((val) {
        ...
        lenthofList = val.approvalsvalue!.length;
        for (int i =0; i<lenthofList; i++ ){
            filtervalue.add(ApprovalsValue(
                              ...
                              child: InkWell(
                                onTap: () {
                                  ...
                                  //  showbottomsheet(context ,i);
                                   ApprovalsInfoState.iscomeFromRejectORApproved=true;
                                  ApprovalsDetailsAPi.draftEntry = filtervalue[i].DraftEntry .toString();
                                  ApprovalsPatchAPi.approvalID = filtervalue[i].WddCode.toString();
                                  ApprovalsInfoState.docTypeName =  filtervalue[i].ObjType.toString().contains('13')?'A/R Invoice':
                                                   filtervalue[i].ObjType.toString().contains('23')?'Sales Quotation':
                                                    filtervalue[i].ObjType.toString().contains('17')?'Sales Order':
                                                     filtervalue[i].ObjType.toString().contains('14')?'Sales Return':
                                                     filtervalue[i].ObjType.toString().contains('15')?'Deliveries':'';
                                          ...
                                          Column(
                                            ...
                                            children: [
                                              SizedBox(
                                                ...
                                                //color: Colors.red,
                                                child: Text(
                                                  filtervalue[i].ObjType.toString().contains('13')?'A/R Invoice':
                                                   filtervalue[i].ObjType.toString().contains('23')?'Sales Quotation':
                                                    filtervalue[i].ObjType.toString().contains('17')?'Sales Order':
                                                     filtervalue[i].ObjType.toString().contains('14')?'Sales Return':
                                                     filtervalue[i].ObjType.toString().contains('15')?'Deliveries':'',
                                                  ...
                                                  SizedBox(
                                                     width: Screens.width(context)*0.7,
                                               // color: Colors.red,
                                                    child: Text("From: ${filtervalue[i].FromUser}",
                                          ...
                                          mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                                children: [
                                                  SizedBox(
                                                   ...
                                                   // color: Colors.red,
                                                    child: Text("${filtervalue[i].cardName}",
                                                     style:TextStyles.bodytextBlack1(context),),
                                                  ),
                                                   // SizedBox(width: Screens.width(context) * 0.1),
                                                   Container(
                                                    child: Text("${filtervalue[i].createDate}",
                                                     style: TextStyles.bodytextBlack1(context),),)
                                                ],
                                              )
                                    ],
