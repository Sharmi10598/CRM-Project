...
class ContentOrderCreation extends StatefulWidget {
  ...
  @override
  ContentOrderCreationState createState() => ContentOrderCreationState();
}
...
class ContentOrderCreationState extends State<ContentOrderCreation> {
              ...
              child: Padding(
                ...
                child: SizedBox(
                  ...
                  child: ElevatedButton(
                      onPressed: () {
                        if (HeaderOrderCreationState.isTextFiledEnabled ==
                            true) {
                          if (HeaderOrderCreationState
                          ...
                          if (HeaderOrderCreationState.bpName != '') {
                            pageController.animateToPage(++pageChanged,
                                duration: Duration(milliseconds: 250),
                                curve: Curves.bounceIn);
                            CreateDetailsState.contentAddItems = true;
  ...
  void ValidateAndCallApi() {
    if (carcode == null) {
      ...
      SalesOrderPostAPi.cardCodePost = carcode;
      SalesOrderPostAPi.cardNamePost = HeaderOrderCreationState.bpName;
      SalesOrderPostAPi.docLineQout = itemsDetails;
  ...
  void setValuesToListPageapproval() {
    ...
    for (int i = 0; i < itemsDetails3.length; i++) {
      ...
      int carton1 = 0;
      log("tax list values: " + taxs.toStringAsFixed(0));
  ...
  void setValuesToListPagequotation2() {
    ...
    for (int i = 0; i < itemsDetails2.length; i++) {
      log('Taxname:::${itemsDetails2[i].taxCode.toString()}');
...
// taxName=
  ...
  void sumofTotal() {
    ...
    double tax = 0;
    double bfd = 0;
    HeaderOrderCreationState.discount = 0;
    HeaderOrderCreationState.tax = 0;
    HeaderOrderCreationState.total = 0;
    HeaderOrderCreationState.totalBeforeDiscount = 0;
    setState(() {
      if (itemsDetails.isNotEmpty) {
...
class AddOrderItem {
  ...
  AddOrderItem(
      ...
      this.basedocentry,
      this.shipToCode,
      this.valueAFdisc = 0.00,
      this.valueBFdisc = 0.00,
      this.carton,
      this.lineNo,
      this.U_Pack_Size,
      this.U_Tins_Per_Box,
  ...
  Map<String, dynamic> tojson2() {
    if (BaseType == null && basedocentry == null && baseline == null) {
    ...
    if (BaseType == -1 || basedocentry == "0" && baseline == "0") {
      Map<String, dynamic> map = {
        "ItemCode": itemCode,
        "ItemDescription": itemName,
        "DiscountPercent": discounpercent.toString(),
