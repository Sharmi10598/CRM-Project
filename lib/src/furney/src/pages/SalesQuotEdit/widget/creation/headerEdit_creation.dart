...
class HeaderEditCreation extends StatefulWidget {
  const HeaderEditCreation({Key? key}) : super(key: key);

  @override
  HeaderEditCreationState createState() => HeaderEditCreationState();
}

class HeaderEditCreationState extends State<HeaderEditCreation> {
    ...
    ContactPersonsAPi.getGlobalData().then((value) {
      if (value.deliveryModaleValue!.length > 0) {
        setState(() {
          ...
          LogisticEditPageState.shipto = value.ShipToDefault.toString();
          LogisticEditPageState.billto = value.BilltoDefault.toString();
        });
      }
    });
  }
...
//   }
  ...
  //   } else if (ContentEditCreationState.itemsDetails.length < 1) {
  //     const snackBar = SnackBar(
  //       duration: Duration(seconds: 5),
  //       backgroundColor: Colors.red,
  //       content: Text(
  ...
  //     SalesQuotPatchAPi.cardCodePost = bpCode;
  //     SalesQuotPatchAPi.docLineQout = ContentEditCreationState.itemsDetails;
  //     SalesQuotPatchAPi.docDate = currentDateTime.toString();
  //     SalesQuotPatchAPi.dueDate = currentDateTime.toString();
  //     SalesQuotPatchAPi.remarks = mycontroller[1].text;
  //     print('remarks: ${SalesQuotPatchAPi.remarks}');
  //     final uuid = Uuid();
