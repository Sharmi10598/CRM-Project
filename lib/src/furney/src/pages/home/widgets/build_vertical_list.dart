      ...
      itemBuilder: (context, i) {
        ...
        return Card(
                              ...
                              child: InkWell(
                                ...
                                ApprovalsDetailsAPi.draftEntry = aproval[i].DraftEntry .toString();
                                ApprovalsPatchAPi.approvalID = aproval[i].WddCode.toString();
                                ApprovalsInfoState.docTypeName =  aproval[i].ObjType.toString().contains('13')?'A/R Invoice':
                                                   aproval[i].ObjType.toString().contains('23')?'Sales Quotation':
                                                   aproval[i].ObjType.toString().contains('17')?'Sales Order':
                                                    aproval[i].ObjType.toString().contains('14')?'Sales Return':
                                                    aproval[i].ObjType.toString().contains('15')?'Deliveries':'';
                                      ...
                                      Column(
                                        ...
                                        children: [
                                                  ...
                                                  SizedBox(
                                                    ...
                                                    //color: Colors.red,
                                                    child: Text(
                                                      aproval[i].ObjType.toString().contains('13')?'A/R Invoice':
                                                       aproval[i].ObjType.toString().contains('23')?'Sales Quotation':
                                                        aproval[i].ObjType.toString().contains('17')?'Sales Order':
                                                         aproval[i].ObjType.toString().contains('14')?'Sales Return':
                                                         aproval[i].ObjType.toString().contains('15')?'Deliveries':'',
                                                      ...
                                                      SizedBox(
                                                         width: Screens.width(context)*0.7,
                                                   // color: Colors.red,
                                                        child: Text("From: ${aproval[i].FromUser}",
