...
class CreateVisitPageState extends State<CreateVisitPage> {
  ...
  Future<void> callPostApi() async {
    ...
    await PostPurposeVisitAPi.mehod(purpVisitModel);
    await PostPurposeVisitAPi.getGlobalData(purpVisitModel).then((value) async {
      LoadingBtn = false;
      setState(() {});
      if (value.statusCode! >= 200 && value.statusCode! <= 210) {
        await sendNotio();
        await GetLeadToken.getGlobalData().then((tokens) async {
          if (tokens != 'Error') {
            final push = PushNotify(
              msg:
                  '${GetValues.userName} created a visit plan to this ${SelectedCustModel.CustomerName} customer..!!',
